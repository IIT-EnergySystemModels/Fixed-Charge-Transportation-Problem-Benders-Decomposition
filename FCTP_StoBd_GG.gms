...
parameters
   ...
   Z2_S   (s)       subproblem objective function value        in iteration l
...
*  using four cores and assignment of scenarios to cores
   ...
   coresc(core,s) cores to scenario / core001.(s000*s024)
                                      core002.(s025*s049)
                                      core003.(s050*s074)
                                      core004.(s075*s099) /
...
*   st_report_o(s,sh) status report
...
*  solving subproblem
   ...
   Subproblem_Bd.SolveLink = %SolveLink.AsyncGrid% ;
*  Sending loop
   loop (core,
      gs(s) = coresc(core,s)
      if (sum[gs(s), 1] > 0,
         solve Subproblem_Bd minimizing Z2 using RMIP scenario scen_dem ;
         pGridHandle(core) = Subproblem_Bd.Handle ;
      ) ;
   ) ;
*  Recovering loop
   repeat
   ...
   Subproblem_Bd.SolveLink = %SolveLink.LoadLibrary% ;
...
*     updating lower and upper bound
      Z_Lower =              Z1.l ;
      Z_Upper = min(Z_Upper, Z1.l - Theta.l + sum[s, P(s) * Z2_s(s)]) ;
